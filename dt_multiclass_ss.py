...
def parse_arguments():
    ...
    args.output_folder = check_dir(os.path.join(args.output_root, 'dt_multiclass', args.exp_name))
...
def main(args):
    ...
    model = Segmentator(pred_chs, pretrained_model.features, img_size=size).cuda()
    ...
    for epoch in range(100):
        ...
        save = (epoch % args.snapshot_freq == 0 and epoch > 0 and val_loss < best_val_loss)
...
def train(loader, model, criterion, optimizer, log, logger):
    ...
    for idx, (img, label) in enumerate(loader):
        ...
        if idx % 50 == 0:
            ...
            scalar_dict = {"loss": loss_meter.mean, "miou":iou_meter.mean, "epoch_prog":idx / (len(loader.dataset) / loader.batch_size)}
...
def validate(loader, model, criterion, log, logger, epoch=0):
    ...
    for idx, (img, label) in enumerate(loader):
        ...
        labels = (torch.nn.functional.interpolate(label.cuda(), size=logits.shape[-2:]).squeeze(1)).long()
    ...
    save_in_log(log, global_step, set="valid", scalar_dict=results)#, text_dict=text_dict)
    logger.info(text_print)
    ...
    if image_dict:
        for k, v in image_dict.items():
            if k=='sample':
                ...
                log.add_images(set+"_"+k, v.unsqueeze(1).float()/v.max().float(), save_step)
            elif k=='pred':
                if len(v.shape)==3:
